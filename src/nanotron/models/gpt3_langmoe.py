...
from nanotron.config import GPT3Config, GPT3LangMoEConfig, ParallelismArgs
from nanotron.generation.generate_store import AttachableStore
from nanotron.models import gpt3
from nanotron.models.gpt3 import (
    CausalSelfAttention,
    GPT3ForTraining,
    GPT3Model,
    dropout_add_fused_train,
)
...
@contextmanager
def replace_moe_decoder(gpt3config: GPT3LangMoEConfig):
    ...
    try:
        ...
        def create_pp_block(module_builder, module_kwargs, **kwargs):
            if module_builder is GPT3Block:
                ...
                return orig(
                    module_builder=GPT3LangMoEBlock,
...
@contextmanager
def replace_gpt3_moe_model(gpt3moeconfig: GPT3LangMoEConfig):
    orig = gpt3.GPT3Model
    try:
        ...
        def create_moe_model(
            ...
            return GPT3LangMoEModel(gpt3moeconfig, parallel_context, parallel_config, random_states)
...
class LanguageEmbedding(nn.Module, AttachableStore):
    def __init__(
        self,
        tp_pg: dist.ProcessGroup,
        config: GPT3LangMoEConfig,
        parallel_config: Optional[ParallelismArgs],
    ):
        super().__init__()
        self.language_embedding = TensorParallelEmbedding(
            num_embeddings=config.num_languages,
            embedding_dim=config.language_embedding_size,
            pg=tp_pg,
    ...
    def forward(
        self,
        lang_code: torch.Tensor,  # [batch_size, 1]
    ):
        lang_code = lang_code.transpose(0, 1)
        lang_emb = self.language_embedding(lang_code)
        return {"lang_emb": lang_emb}


class GPT3LangMoEBlock(nn.Module):
    def __init__(
        self,
        config: GPT3LangMoEConfig,
        parallel_config: Optional[ParallelismArgs],
        ...
        super(GPT3LangMoEBlock, self).__init__()
    ...
    def forward(
        ...
        lang_emb: torch.Tensor | TensorPointer,
        ...
        mlp_output = self.ff(hidden_states=hidden_states, lang_hidden_states=lang_emb)
...
class GPT3LangMoEModel(GPT3Model):
    def __init__(
        self,
        config: GPT3LangMoEConfig,
        parallel_context: ParallelContext,
        ...
        self.language_embeddings = PipelineBlock(
            p2p=self.p2p,
            module_builder=LanguageEmbedding,
            module_kwargs={
                "tp_pg": parallel_context.tp_pg,
                "config": config,
                "parallel_config": parallel_config,
            },
            module_input_keys={"lang_code"},
            module_output_keys={"lang_emb"},
        )
    ...
    def forward(
        ...
        lang_code: torch.Tensor | TensorPointer,  # [batch_size, 1]
        aux_losses: Dict[str, Union[torch.Tensor, TensorPointer]],
    ):
        ...
        # language embedding for MoE
        lang_emb = self.language_embeddings(lang_code=lang_code)["lang_emb"]
        ...
        for encoder_block in self.decoder:
            hidden_encoder_states = encoder_block(**hidden_encoder_states, lang_emb=lang_emb)
        ...
        return {
            "sharded_logits": fp32_sharded_logits,
            "aux_losses": hidden_encoder_states["aux_losses"],
        }


class GPT3LangMoEForTraining(GPT3ForTraining):
    def __init__(
        self,
        config: GPT3LangMoEConfig,
        parallel_context: ParallelContext,
    ...
    def get_block_compute_costs(self):
        ...
        block_compute_costs = {
            ...
            GPT3LangMoEBlock: att_cost + mlp_cost,
